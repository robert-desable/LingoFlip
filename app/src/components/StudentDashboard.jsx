...
import { Hand, Volume2, AlertTriangle, ArrowLeft, LogOut } from 'lucide-react';
import { AdultTeacherIcon, ChildStudentIcon } from './icons/RoleIcons';
import { translateText, processAudioPipeline } from '../services/ai4bharat';
...
function StudentDashboard() {
  ...
  useEffect(() => {
    socket.on('receive-transcript', async (transcript) => {
      const srcLang = transcript.originalLang || teacherLanguage;
      setOriginalSpokenText(transcript.text);

      // Show translating status with mother tongue primary
      setIncomingText(`[${t.translatingText}]...`);
      
      const translated = await resolveTranslation(transcript.text, srcLang, motherTongue);
      setIncomingText(translated);
    });

    socket.on('room-language-updated', ({ language }) => {
        ...
        <div className="max-w-3xl w-full text-center">
          {incomingText ? (
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-sky-50 dark:bg-sky-950/60 border border-sky-100 dark:border-sky-900/40 rounded-full flex-wrap justify-center">
              ...
              {originalSpokenText && (
                <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-100 dark:border-slate-800 max-w-xl mx-auto">
                  <p className="text-xs text-slate-400 dark:text-slate-500 font-semibold mb-0.5">
                    Teacher Spoke ({teacherLanguage === 'en' ? 'English' : 'Hindi'}):
                  </p>
